...
*/
import Node from "./components/dom/Node.js";
 ...
 */
    ...
    static registerComponent(instance, name) {
        if( window.componentList["components"][name] === undefined ) {
            window.componentList["components"][name] = instance;
        }
    }

    static registerDirective(instance, name) {
        if( window.componentList["directives"][name] === undefined ) {
            window.componentList["directives"][name] = instance;
        }
    }
    ...
    init() {
        if(!this.initialized) {
            this.context.find("[data-component], [data-directive]").each(element => {
                let functionMap = {};
                ...
                //loop through the array that "key" holds
                    ...
                    //@TODO: fix weird empty component,
                        ...
                        //no module was found, try to fetch it from the array
                            if( window.componentList[name][functionMap[name][i]] !== undefined ) {
                                new window.componentList[name][functionMap[name][i]](element, Components);
                            }
                            else {
                                console.error(err);
